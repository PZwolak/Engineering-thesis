...
$wilgg="SELECT wilg, date FROM tiwsql WHERE date BETWEEN '{$czass}' AND '{$czas}'";

$tabela3=mysqli_query($mysqli, $wilgg);
	
	$licznik=0;
	    while($row =mysqli_fetch_assoc($tabela3))
    {
        $wilg[] = (float)$row['wilg'];
        $dzis[] = $row['date'];
		$licznik=$licznik+1;
    }
	$licznik2xx=$licznik/25;
